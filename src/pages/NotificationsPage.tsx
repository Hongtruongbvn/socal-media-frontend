...
export default function Notifications() {
  ...
  const renderNotificationMessage = (noti: Notification) => {
    ...
    switch (noti.type) {
      case 'NEW_REACTION': return `${senderName} đã bày tỏ cảm xúc với bài viết của bạn`;
      case 'NEW_COMMENT': return `${senderName} đã bình luận về bài viết của bạn`;
      case 'NEW_FOLLOWER': return `${senderName} đã bắt đầu theo dõi bạn`;
      case 'FRIEND_REQUEST': return `${senderName} đã gửi lời mời kết bạn`;
      case 'FRIEND_ACCEPTED': return `${senderName} đã chấp nhận lời mời kết bạn của bạn`;
      case 'GAME_INVITE': return `${senderName} đã mời bạn chơi ${noti.metadata?.gameName || 'một trò chơi'}`;
      case 'GROUP_INVITE': return `${senderName} đã mời bạn tham gia nhóm ${noti.metadata?.groupName || ''}`;
      case 'GROUP_REQUEST_ACCEPTED': return `Yêu cầu tham gia nhóm của bạn đã được chấp nhận`;
      case 'GROUP_REQUEST_REJECTED': return `Yêu cầu tham gia nhóm của bạn đã bị từ chối`;
      case 'POST_DELETED_BY_ADMIN': return `Bài viết của bạn đã bị xóa bởi quản trị viên`;
      case 'WARN': return `${senderName} đã cảnh cáo bạn`;
      default: return `${senderName} đã gửi cho bạn một thông báo`;
    }
  };
  ...
  const renderWarningDetails = (noti: Notification) => (
    <div className="warning-details">
      {noti.metadata?.reportReason && (
        <div><strong>📋 Nguyên nhân báo cáo:</strong> {noti.metadata.reportReason}</div>
      )}
      ...
      {noti.metadata?.reason && (
        <div><strong>⚠️ Lý do cảnh cáo:</strong> {noti.metadata.reason}</div>
      )}
    </div>
  );
    ...
    <div className="notifications-page">
      <div className="section-container">
        <h2 className="section-title">
          ⚠️ Cảnh Báo Của Bạn
          {warnings.length > 0 && (
            <span className="clear-all" onClick={() => handleClearAll('warnings')}>🗑️ Xóa tất cả</span>
          )}
        </h2>
        {warnings.length === 0 ? (
          ...
          <ul className="notification-list">
            {warnings.map((warn) => (
              <li key={warn._id} className="notification-item warning-item">
                <img src={asAvatar(pickAvatar(warn.sender))} alt={warn.sender?.username} onError={(e) => { (e.currentTarget as HTMLImageElement).src = AVATAR_FALLBACK; }} />
                ...
                <div className="notification-actions">
                  <button onClick={(e) => { e.stopPropagation(); handleDeleteWarning(warn._id); }} title="Xóa cảnh báo">
                    <TrashIcon size={18} />
                  </button>
                </div>
              </li>
      ...
      <div className="section-container">
        <h2 className="section-title">
          ...
          {friendRequests.length > 0 && (
            <span className="clear-all" onClick={() => handleClearAll('friendRequests')}>🗑️ Xóa tất cả</span>
          )}
        </h2>
        {friendRequests.length === 0 ? (
          ...
          <ul className="notification-list">
            {friendRequests.map((req) => (
              <li key={req._id} className="notification-item">
                <img src={asAvatar(pickAvatar(req.sender))} alt={req.sender.username} onError={(e) => { (e.currentTarget as HTMLImageElement).src = AVATAR_FALLBACK; }} />
                <div className="notification-content">
                  <span className="notification-text"><strong>{req.sender.username}</strong> đã gửi cho bạn lời mời kết bạn</span>
      ...
      <div className="section-container">
        <h2 className="section-title">
          ...
          {notifications.length > 0 && (
            <span className="clear-all" onClick={() => handleClearAll('notifications')}>🗑️ Xóa tất cả</span>
          )}
        </h2>
        {notifications.length === 0 ? (
          ...
          <ul className="notification-list">
            {notifications.map((noti) => (
              <li key={noti._id} className={`notification-item ${!noti.isRead ? 'unread' : ''}`} onClick={() => handleMarkAsRead(noti._id)}>
                <img src={asAvatar(pickAvatar(noti.sender))} alt={noti.sender?.username} onError={(e) => { (e.currentTarget as HTMLImageElement).src = AVATAR_FALLBACK; }} />
                <div className="notification-content">
                  ...
                  {noti.type === 'GROUP_INVITE' && noti.metadata?.inviteId && (
                    <div className="action-buttons">
                      <button onClick={(e) => { e.stopPropagation(); respondToGroupInvite(noti.metadata!.inviteId!, 'ACCEPT', noti._id); }} className="accept-btn">✅ Tham gia</button>
                      <button onClick={(e) => { e.stopPropagation(); respondToGroupInvite(noti.metadata!.inviteId!, 'DECLINE', noti._id); }} className="reject-btn">❌ Từ chối</button>
                    </div>
                  )}
                </div>
                <div className="notification-actions">
                  <button onClick={(e) => { e.stopPropagation(); handleDeleteNotification(noti._id); }} title="Xóa thông báo">
                    <TrashIcon size={18} />
                  </button>
                </div>
              </li>
